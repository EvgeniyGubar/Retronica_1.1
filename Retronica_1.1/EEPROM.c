...
{	
	//ѕам€ти не хватило, еепром надо зашивать отдельно
		...
		// 		}
		// 		eeprom_write_byte(& setting_eeprom[12], 1);	//отображение температуры
		// 		eeprom_write_byte(& setting_eeprom[13], 1);	//отображение даты
		...
		// 		
		// 		eeprom_write_byte(& setting_eeprom[18], 1);	//отображение давлени€
		// 		
	...
	Time.press_on = eeprom_read_byte(& setting_eeprom[18]);
...
void save_to_eeprom(void)
{
	...
	if (Time.press_on != eeprom_read_byte(& setting_eeprom[18]))	eeprom_write_byte(& setting_eeprom[18], Time.press_on);
